...
<head>
    ...
    <style>
        ...
        * {{ margin: 0; padding: 0; box-sizing: border-box; font-family: 'Inter', sans-serif; transition: background-color 0.3s, color 0.3s; }}
        ...
        .header-logo {{ height: 48px; width: auto; object-fit: contain; padding: 4px 8px; border-radius: 8px; background: rgba(255,255,255,0.95); }}
        [data-theme="light"] .header-logo {{ background: transparent; }}
        .header-title {{ }}
...
<body>
    ...
    <script>
        ...
        function initCharts() {{
            ...
            cNodo = new Chart(document.getElementById('cNodo'), {{ type: 'bar', data: {{ labels: ["OLT", "ODF", "SW", "ITM (x2)"], datasets: [{{ label: 'Instalados', data: [], backgroundColor: '#22d3ee', borderRadius: 4 }}, {{ label: 'Faltantes', data: [], backgroundColor: '#f43f5e', borderRadius: 4 }}] }}, options: {{ responsive: true, maintainAspectRatio: false, plugins: {{ legend: {{ position: 'bottom', labels: {{ padding: 10, boxWidth: 12 }} }}, datalabels: {{ ...dlOpts, display: (ctx) => ctx.dataset.data[ctx.dataIndex] > 0 }} }}, scales: {{ x: {{ stacked: true, grid: {{ display: false }} }}, y: {{ stacked: true, ticks: {{ precision: 0, stepSize: 1 }}, grid: {{ color: getGridColor() }} }} }} }} }});
        ...
        function updateDashboard() {{
            ...
            // OLT, ODF, SW = 1 por localidad. ITM = 2 por localidad
            let totalITMEsperados = fNodo.length * 2;
            let totalEquiposEsperados = fNodo.length * 3 + totalITMEsperados;
            ...
            fNodo.forEach(n => {{
                ...
                let itmVal = parseInt(n.ITM_Instalados) || 0;
                nITM += itmVal; equiposInstalados += itmVal;
            }});
            ...
            cNodo.data.datasets[0].data = [nOLT, nODF, nSW, nITM]; cNodo.data.datasets[1].data = [fNodo.length - nOLT, fNodo.length - nODF, fNodo.length - nSW, totalITMEsperados - nITM]; cNodo.update();
